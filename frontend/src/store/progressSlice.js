...
import ProgressChartDataService from "../services/progresschart.service";

export const retrieveProgressChart = createAsyncThunk(
  "progresschart/retreive",
  async () => {
    const res = await ProgressChartDataService.getAll();
    // console.log(res);
...
export const progressSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(retrieveProgressChart.pending, (state) => {
      state.loading = true;
    });

    builder.addCase(retrieveProgressChart.fulfilled, (state, action) => {
